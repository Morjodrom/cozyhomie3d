...
test('inherits pot settings, restores tray controls, and exports both printable parts', async ({ page }) => {
  ...
  await expect(page.getByLabel('Beam count')).toHaveValue('4')
  await expect(page.getByLabel('Beam width')).toHaveValue('5')
  await expect(page.getByLabel('Beam length')).toHaveValue('15')
  await expect(page.getByLabel('Model-side height')).toHaveValue('0')
  await expect(page.getByLabel('Outer-side height')).toHaveValue('3')
  await expect(page.getByLabel('Breakaway distance')).toHaveValue('0.05')
  ...
  await page.getByLabel('Beam count').selectOption('8')
  await page.getByLabel('Beam width').fill('4.5')
  await page.getByLabel('Beam length').fill('19')
  await page.getByLabel('Model-side height').fill('2')
  await page.getByLabel('Outer-side height').fill('4')
  await page.getByLabel('Breakaway distance').fill('-0.1')
  await page.waitForTimeout(400)
  await page.reload()
  ...
  await expect(page.getByLabel('Beam count')).toHaveValue('8')
  await expect(page.getByLabel('Beam width')).toHaveValue('4.5')
  await expect(page.getByLabel('Beam length')).toHaveValue('19')
  await expect(page.getByLabel('Model-side height')).toHaveValue('2')
  await expect(page.getByLabel('Outer-side height')).toHaveValue('4')
  await expect(page.getByLabel('Breakaway distance')).toHaveValue('-0.1')
